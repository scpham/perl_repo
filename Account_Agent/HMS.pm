...
$Id: HMS.pm,v 1.54 2006/08/15 21:51:06 scpham Exp $
$Revision: 1.54 $
$Date: 2006/08/15 21:51:06 $
$Author: scpham $
...
sub new() {
  my $class = shift;
  my $self  = {
    'timeout'    => $afa_config{hms_timeout},
    'logger'     => get_logger('AFA::HMS'),
    'verbose'    => undef,
    'writer'     => 1,
    'reader'     => 1,
    'error'      => 1,
...
sub _init() {
  my $self = shift;
  $self->{logger}->debug('Init AFA::HMS');
  $self->_BootStrap();
}
...
sub _LoadSSH() {
  ...
  @{ $self->{_sshargs} } = (
    ...
    "BatchMode=yes",                      $host,
    'PATH=/usr/cisco/bin:/usr/local/bin:/usr/bin:/bin:\$PATH; export PATH;',
    'perl', "-e",
    "$self->{_bootstrap}"
  );
